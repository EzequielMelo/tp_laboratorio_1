...
int ll_clear(LinkedList* this)
{
    ...
    if(this!=NULL)
    {
    	...
    	else //Si no hay elementos dejo el nodo apuntando a NULL
    	{
    ...
    if(this != NULL && this2 != NULL)
    {
    	...
    	if(!ll_isEmpty(this2) && !ll_isEmpty(this))//Verifico que las listas no esten vacias
    	{
    		for(i=0; i<ll_len(this2); i++)//Recorro this2
    		{
    			...
    			if(pElement != NULL)
    			{
    				if(!ll_contains(this, pElement))//Comp
    				{
    					returnAux = 0;
    					break;
    				}
    ...
    if(this != NULL && (order==1 || order==0) && pFunc != NULL)
    {
    	for(i=0; i<ll_len(this)-1; i++)
    	{
    		for(j=i+1; j<ll_len(this); j++)
    		{
    			...
    			if(pFunc(pElementI, pElementJ) > 0 && order == 1)//De forma ascendente
    			{
    			...
    			{
    				if(pFunc(pElementI, pElementJ) <= 0 && order == 0)//De forma descendente
    				{
        				ll_set(this, i, pElementJ);
        				ll_set(this, j, pElementI);
        				returnAux = 0;
